...
[`terraform-modules//modules/aws/vlinder_auth`](https://github.com/vln-devsecops/terraform-modules/tree/main/modules/aws/vlinder_auth).

## Why this repo exists

`vlinder_auth` is a self-provisioning Terraform module: a consumer supplies an
...
| `packages/lambda-src` | Post-confirmation and pre-token-generation Cognito triggers, and the admin-api Lambda handlers. Vendored into `vlinder_auth`'s `lambda-src/`. |
| `packages/admin-panel-site` | The small static admin panel bundled by `vlinder_auth`. Vendored into the module's `admin-panel-site/`. |
| `e2e` | Cucumber + Playwright BDD suite exercising the whole stack (this repo's packages, deployed via a `vlinder_auth` example) end to end. |
