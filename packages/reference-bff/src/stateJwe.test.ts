...
describe('stateJwe', () => {
  it('round-trips a payload through mint and verify', async () => {
    ...
    const token = await mintState({ codeVerifier: 'verifier-value', issuedAt: now }, KEY, 300, now)
    const payload = await verifyState(token, KEY, now)
    expect(payload).toEqual({ codeVerifier: 'verifier-value', issuedAt: now })
  })

  it('rejects a token past its TTL', async () => {
    const now = Date.parse('2026-01-01T00:00:00Z')
    const token = await mintState({ codeVerifier: 'v', issuedAt: now }, KEY, 300, now)
  ...
  it('throws loudly on a key of the wrong byte length, naming the env var', async () => {
    await expect(mintState({ codeVerifier: 'v', issuedAt: Date.now() }, 'too-short', 300)).rejects.toThrow(
      /STATE_JWE_KEY must be exactly 32 bytes/,
    )
  })
  ...
  it('verifyState also throws loudly on a wrong-length key, rather than swallowing it as an invalid token', async () => {
    ...
    const token = await mintState({ codeVerifier: 'v', issuedAt: Date.now() }, KEY, 300)
